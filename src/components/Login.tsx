...
import { ShieldCheck, Lock, User, Sparkles, Sun, Moon } from 'lucide-react';
...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  const { theme, setTheme, triggerToast } = useDashboard();
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-100 via-sky-50 to-slate-200 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 p-4 relative overflow-hidden transition-colors duration-200">
      ...
      <div className="w-full max-w-md p-8 rounded-2xl glass shadow-2xl relative border border-white/20 dark:border-slate-800/50">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {error && (
            <div className="p-3.5 text-sm text-rose-600 bg-rose-50 dark:bg-rose-950/30 dark:text-rose-400 rounded-lg border border-rose-200 dark:border-rose-900/30 animate-shake">
              {error}
            </div>
          )}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="admin"
                className="w-full pl-10 pr-4 py-3 bg-white/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-slate-900 dark:text-white transition-all text-sm"
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3 bg-gradient-to-r from-sky-500 to-indigo-600 text-white font-semibold rounded-xl shadow-lg shadow-sky-500/20 hover:shadow-indigo-500/30 hover:scale-[1.02] focus:scale-95 active:scale-95 disabled:opacity-75 disabled:scale-100 transition-all cursor-pointer text-sm flex items-center justify-center gap-2"
          >
            {isLoading ? (
        ...
        <div className="mt-8 text-center border-t border-slate-200/50 dark:border-slate-800/50 pt-4">
          <p className="text-xs text-slate-400">
            Demo credentials: <code className="bg-slate-200/50 dark:bg-slate-800 px-1.5 py-0.5 rounded text-sky-600 dark:text-sky-400 font-mono">admin</code> / <code className="bg-slate-200/50 dark:bg-slate-800 px-1.5 py-0.5 rounded text-sky-600 dark:text-sky-400 font-mono">admin123</code>
          </p>
        </div>
      </div>
    </div>
